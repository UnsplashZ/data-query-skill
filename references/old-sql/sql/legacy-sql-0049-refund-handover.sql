...
(
    SELECT
        ...
        CASE
            WHEN multiMatchAny(ifNull(a2.class_camp_name, ''), ['延期|冻课']) OR a2.stop_study_status = 1 THEN '已休学'
            ELSE '未休学'
        END AS `休学状态`,
        CASE
    ...
    (
        SELECT
            ...
            h.stop_study_status AS stop_study_status
        FROM
        (
            SELECT
                order_no AS flow_no,
                ...
                argMax(stop_study_status, id) AS stop_study_status
    ...
    (
        ...
        WHERE study_time > 0
          AND multiMatchAny(course_name, ['.*月夜.*上', '.*小白杨.*上', '.*鸿雁.*上', '我和你.*上', '红河谷.*第1课', '手指技术专项强化课.*第一节', '.*欢乐颂.*', '.*先导课.*', '.*楷第1，2课一日千里.*', '.*楷第3，4课天人合一.*'])
...
AND `flow_no` NOT IN ('k2026040909310864166388','k202604021552182093845233','k2026041900093453950313','k202604071811123724012486')
ORDER BY `支付时间` DESC
