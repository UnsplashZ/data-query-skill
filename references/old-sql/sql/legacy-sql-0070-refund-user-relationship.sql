...
SELECT
    ...
    if(ifNull(st.all_started_class_cnt, 0) > 0, '是', '否') AS is_started_class,
    st.first_started_class_time AS first_started_class_time,
    if(ou.has_official_order = 1, '是', '否') AS is_in_official_course,
    round(ou.official_paid_amount, 2) AS official_paid_amount,
...
FROM first_lt1000_user fu
INNER JOIN order_user ou ON fu.union_id = ou.union_id
LEFT JOIN live_user lu ON fu.union_id = lu.union_id
LEFT JOIN study_30 st ON fu.union_id = st.union_id
LEFT JOIN all_friend af ON fu.union_id = af.union_id
ORDER BY ou.latest_pay_time DESC, ou.union_id, friend_status DESC, af.company, af.emp_name
