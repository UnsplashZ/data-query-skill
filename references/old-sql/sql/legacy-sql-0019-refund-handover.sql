...
(
    ...
    (
        ...
        WHERE T3.first_level_name = '课程'
          ...
          AND round(ifNull(T1.total_price, 0) / 100, 4) >= 1880
          AND toDate(T1.pay_time) >= toDate('2025-01-01')
          AND toDate(T1.pay_time) <= toDate('2030-12-31')
    ) a1
    LEFT JOIN
    (
        SELECT
